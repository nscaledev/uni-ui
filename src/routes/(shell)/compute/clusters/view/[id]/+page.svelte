<script lang="ts">
	...
	function startMachine(
		...
		Clients.compute()
			.apiV1OrganizationsOrganizationIDProjectsProjectIDClustersClusterIDMahcinesMachineIDStartPost(
				parameters
			)
			.catch((e: Error) => Clients.error(e));
	}
	...
	function stopMachine(
		...
		Clients.compute()
			.apiV1OrganizationsOrganizationIDProjectsProjectIDClustersClusterIDMahcinesMachineIDStopPost(
				parameters
			)
			.catch((e: Error) => Clients.error(e));
	}
	...
	function softRebootMachine(
		...
		Clients.compute()
			.apiV1OrganizationsOrganizationIDProjectsProjectIDClustersClusterIDMahcinesMachineIDSoftrebootPost(
				parameters
			)
			.catch((e: Error) => Clients.error(e));
	}
	...
	function hardRebootMachine(
		...
		Clients.compute()
			.apiV1OrganizationsOrganizationIDProjectsProjectIDClustersClusterIDMahcinesMachineIDHardrebootPost(
				parameters
			)
			.catch((e: Error) => Clients.error(e));
	}
			...
			{#each data.cluster.status.workloadPools || [] as pool}
				{#each pool.machines || [] as machine}
					<ShellListItem>
							...
							<ShellListItemBadges>
								{#snippet extra()}
									...
									<Badge
										...
										iconcolor="dark:text-surface-500">{machine.status}</Badge
									>
								{/snippet}
							</ShellListItemBadges>
						{/snippet}
						...
						{#snippet trail()}
							<ModalIcon
								icon={machine.status !== Compute.ComputeClusterMachineStatusStatusEnum.Stopped
									? 'mdi:stop'
									: 'mdi:play'}
								label={machine.status !== Compute.ComputeClusterMachineStatusStatusEnum.Stopped
									? 'Stop'
									: 'Start'}
								title="Are you sure?"
								confirm={() =>
									machine.status !== Compute.ComputeClusterMachineStatusStatusEnum.Stopped
